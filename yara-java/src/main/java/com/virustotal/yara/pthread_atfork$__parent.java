...
public interface pthread_atfork$__parent {
    ...
    static MemorySegment allocate(pthread_atfork$__parent fi, SegmentScope scope) {
        return RuntimeHelper.upcallStub(pthread_atfork$__parent.class, fi, constants$86.pthread_atfork$__parent$FUNC, scope);
    }
    static pthread_atfork$__parent ofAddress(MemorySegment addr, SegmentScope scope) {
        MemorySegment symbol = MemorySegment.ofAddress(addr.address(), 0, scope);
        return () -> {
            try {
                constants$86.pthread_atfork$__parent$MH.invokeExact(symbol);
            } catch (Throwable ex$) {
